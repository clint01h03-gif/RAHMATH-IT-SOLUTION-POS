...
loginForm.addEventListener('submit', function(e) {
  ...
  if (found) {
    ...
    setTimeout(function() { drawPLChart(); renderDashRecent(); }, 300);
  } else {
...
document.querySelectorAll('.nav-item').forEach(function(item) {
  item.addEventListener('click', function() {
    ...
    if (panel === 'settings') { renderUsers(); refreshGdriveUI(); }
    if (panel === 'report') renderReports();
    if (panel === 'dashboard') { drawPLChart(); renderDashRecent(); }
  });
});
...
function calcBill() {
  ...
  document.getElementById('bill-sub').textContent = '₹' + total.toLocaleString('en-IN');
...
function renderSvcPicker(filter) {
  ...
  active.forEach(function(s) {
    ...
    item.innerHTML =
      ...
      '<div class="svc-pick-price">₹' + Number(s.price).toLocaleString('en-IN') + '</div>';
    item.addEventListener('click', function() {
...
function buildPrintHTML(opts) {
  ...
  items.forEach(function(it, i) {
    ...
    rows += '<tr style="background:' + bg + ';">' +
      ...
      '<td style="padding:12px 10px;border-bottom:1px solid #f0e6d0;text-align:right;color:#333;">₹' + Number(it.rate).toLocaleString('en-IN') + '</td>' +
      '<td style="padding:12px 10px;border-bottom:1px solid #f0e6d0;text-align:right;font-weight:600;color:#1a1200;">₹' + Number(it.amount).toLocaleString('en-IN') + '</td></tr>';
  });
  ...
  return '<div class="inv-sheet" style="font-family:Inter,Segoe UI,Arial,sans-serif;max-width:680px;margin:0 auto;background:#ffffff;color:#111;box-shadow:0 8px 40px rgba(0,0,0,0.08);">' +
    ...
    '<div style="display:flex;justify-content:space-between;font-size:13px;color:#b7c6e6;margin-bottom:8px;"><span>Subtotal</span><span>₹' + total.toLocaleString('en-IN') + '</span></div>' +
    ...
    '<span style="font-size:22px;font-weight:800;color:#ffd88a;">₹' + total.toLocaleString('en-IN') + '</span>' +
    '</div></div></div>' +
    /* footer */
...
function renderReports() {
  ...
  document.getElementById('rep-sales').textContent = '₹' + allTotal.toLocaleString('en-IN');
  document.getElementById('rep-paid').textContent = '₹' + paidTotal.toLocaleString('en-IN');
  document.getElementById('rep-unpaid').textContent = '₹' + unpaidTotal.toLocaleString('en-IN');
...
if (btnReset) {
  btnReset.addEventListener('click', function() {
    ...
    try {
      ...
      // keep users + reset password + gdrive flag
    } catch (e) {}
    services = defaultServices();
    saveServicesData();
    renderServices();
    ...
    showToast('System data reset complete');
    renderDashRecent();
    drawPLChart();
  });
}
...
loadUsers();
refreshGdriveUI();
setTimeout(function() { drawPLChart(); renderDashRecent(); }, 400);
